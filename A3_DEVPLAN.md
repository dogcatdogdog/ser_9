...
**定稿策略 (W10-W13)**: 材料驱动决策 —
  开发全程沉淀可复用材料（对比实验 / 消融 / 扩展实验），不预判最终形态；
  定稿时按手上材料的支持度选择交付形态（论文 或 专利，二选一，不强制双交付），
  **专利交底书为底线**，材料冗余只多不少（8/19 定稿）
...
## W10-W13: 材料储备 + 定稿（形态由材料决定）

> 策略（8/19 定稿）: 不预判最终形态。W10-W11 继续储备材料（对论文和专利
> 实施例均有效），W12 按材料支持度选定形态，W13 定稿。**底线: 专利交底书**。

### W10: 实验设计（材料储备）

- [ ] 对比基线选定（支撑论文对比 / 专利实施例皆可）
- [ ] 时间窗支持（进阶，可选）
- [ ] 实验能证伪创新点（等效变换 / 耦合模型 / 增量评估）

### W11: 对比 + 消融（材料储备）

- [ ] 对比实验 (vs OR-Tools, vs PyVRP, vs VeRyPy heuristics) — 材料通用
- [ ] 消融实验 — 材料通用
- [ ] 显著性检验 — 仅论文形态需要；若定稿为专利可不做

### W12: 成稿（按材料选择形态）

- [ ] 材料盘点: 对比/消融/扩展数据 → 判断支撑论文还是专利
- [ ] 按选定形态成稿: 论文 或 专利交底书（不强制两者）
- [ ] 写作质量检查

### W13: 定稿

- [ ] 底线交付: 专利交底书定稿（可申报）
- [ ] 若材料支持: 论文定稿（可投递），专利作为冗余保留
- [ ] 结题
...
| 定稿 | W13 | 10/30 | 底线: 可申报专利；材料支持则另加可投递论文 |
