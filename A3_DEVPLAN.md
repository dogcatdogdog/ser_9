...
> 母版定为**矿业式 8 节**（非原计划的 6 章节）；前期准备与初稿已完成，
> 详见 `docs/patent/`（TEMPLATE/PRIOR_ART_SURVEY/OUTLINE/SCENARIO/FIGURES/WRITING_PLAN/DATA_CHECK）
> 与 `docs/patent_disclosure.md`。

- [x] W8 前期准备 (9/2 完成):
      - 母版确定 = 矿业式 8 节 (含【提示】指引)；同类专利调研 (载重-能耗耦合前案)
      - 写作大纲 (章节/小节/图位/字数)、场景规格 (MVP↔实验↔结果对照 + 边界红线)
      - 写作计划 (内容需求/字数预算/写作顺序/R0-R6 流程/自检清单/数据核对机制)
      - 图集 9 张 (TikZ, 中文标注/黑白线条, `docs/patent/tikz/` 编译产出 `pdf/`)
- [x] 专利交底书初稿 (`docs/patent_disclosure.md`, 8 节 + 附录, 约 1 万字):
      1. 背景技术 (现有技术缺陷: 目标与可行性解耦; 忽略载重-能耗耦合)
      2. 技术方案 (S1 等效电量距离变换 → S2 电量感知构造 → S3 增量搜索 → S4 后验证)
      3. 关键创新点 (等效电量距离变换 + 载重-能耗耦合模型 + 增量电量校验)
      4. 实施例 (93 实例: 自建 5/10/15/20 + 电量紧张 + Solomon 前20; 含可重现走通实例)
      5. 有益效果 (Solomon 最大省电 16.2%; 随机 20 点均值 5.24%; 零不可行; 3.0× 增量加速)
- [ ] 初稿自检与审阅 (R2 自检清单 / R3 数据核对 逐行打勾 / R5 代理人与导师审阅)
...
**阶段**: W1 ✅ → W2 ✅ → W3 ✅ → W4 ✅ → W5 月1中检 ✅ → W6 Rust 骨架 ✅ → W7 纯函数+服务 ✅
→ W8 交底书初稿 ✅ (经 R2/R3/R5 审阅与实审模拟修订) → W9 精确解扩至 n≤20 + 自适应模式 ✅
+ 交底书全面改写 ✅ (9/10)
**阻塞**: 无
**下一步 (W10)**:
  - 交底书交代理师评估创造性 —— 已知技术已主动承认, 主张落在"两模式互为支撑"的组合效果
  - W10 材料完善 (可选, 视代理师反馈): 若需强化"可行性"主张, 可补"约束绑定且有对照"的实例组
  - 代码清理 + 注释整理
**备注 (W9)**: `plan_multistop_adaptive` 只支持 1 ≤ n ≤ 20 (两种模式同受
  `solver.MAX_TARGETS` 约束); 若实施例需要 n > 20 的对照, 需另行放开启发式上限。
  质量界为"由可采纳下界算出的真实上界", 对任意实例成立但较松 (均值约 72%), 交底书已如实标注。
