...
```
方法对比 (circle 分布, scale=1000, seed=42):

  n   CP-SAT 耗时   CP-SAT == Held-Karp?   能量感知 DP 耗时   能量 DP == 暴力枚举?
  5    0.43s              ✅ 一致                0.00s                ✅ 一致
 10    0.03s              ✅ 一致                0.04s                ✅ 一致
 15    0.03s              ✅ 一致                1.87s                ✅ 一致
 20    0.07s              ✅ 一致 (status=OPTIMAL)   —              —

基线路线在真实约束下的可行性 (关键论据, 分两类约束):
  载重约束: 默认 demand (1,10) 总需求 63.8kg > 50kg 容量
    n=10: 几何最优 ✗ (载重超限) | 能量 DP 最优 ✗
  电量约束 (tight 区域: scale=3000, 电池 3500Wh):
    seed=42: 几何最优 ✗ (电池耗尽) | 我们的约束感知解 ✅ 可行
    seed=0/1/2/7/123: 几何最优 ✅ (电量未绑定)
  → 两种约束都可能让标准求解器的"最优解"在真实模型下不可行
```

**调研结论（已确定）:**
```
✅ 精确解方案: OR-Tools CP-SAT 电路约束 (AddCircuit) — n≤20 全部 OPTIMAL
   - 与 Held-Karp DP (自写, 验证 CP-SAT 正确性) 在 n=5/10/15 上完全一致
   - 与暴力枚举在 n=4 上完全一致 (两种 DP 均验证)
   - 距离 ×100 取整 (整数线性规划), 容差 < 0.5 判定一致
   - 注意: num_workers=1 保证确定性 (多 worker 下 CP-SAT 结果可能不稳定)
✅ 能量感知精确最优: 状态相关边权 DP — dp[mask][j], 边权由已访问集合决定
   (payload = total − Σdemand(mask)), n≤15 时 <2s, 作为能量目标下的精确基线
   - 这是"无电池约束"的精确最优; 电池约束下的精确最优需分支定界,
     留待论文期 (W10+) — 当前以"最优序列的电池可行性检查"作为补充指标
✅ gap 公式: gap = (our_cost − opt_cost) / opt_cost × 100%
   双基线: (1) gap_geo = 我们的总几何距离 vs OR-Tools 几何最优
           (2) gap_equiv = 我们的总等效距离 vs 能量感知 DP 精确最优
           每个基线同时记录其路线在真实电量模型下的可行性
✅ 关键发现 (专利论据): 标准 TSP/VRP 求解器 (OR-Tools/PyVRP) 的
   "几何最优"在真实约束下可能不可行 —
   (1) 载重: 默认 demand 下总需求超容量 (63.8kg > 50kg)
   (2) 电量: tight 区域 (大尺度/小电池) 下电池耗尽 (seed=42 实测)
   → 电量/载重约束必须纳入优化目标, 而非事后校验
   同时: 能量感知优化下我们的等效成本普遍 ≤ 几何最优路线的等效成本
   (实测 Solomon R101/C101/RC101 上 gap_eng 为 −2%~−16%, 即我们更省电)
✅ Solomon BKS: fixtures 是截取前 20 点的自定义子集 (id 已重命名),
   文献无此截断子集的 BKS → 用 OR-Tools CP-SAT 求 20 点几何最优作为 BKS
```
...
**调研结论（已确定）:**
```
✅ 消融变体 6 个 (a3_python/ablation.py, 组合现有纯函数, 不改 plan_multistop 签名):
   1. full          = 完整方法 (NN 构造 + VND 搜索)          — 基准
   2. nn_only       = 仅 NN 构造, 不搜索                    — 量化搜索贡献
   3. no_oropt      = NN + 仅 2-opt 搜索                    — 量化 Or-opt 贡献
   4. no_2opt       = NN + 仅 Or-opt 搜索                   — 量化 2-opt 贡献
   5. fixed_payload = β=0 构造+搜索 (忽略载重耦合), 真实 β 评估 — 量化创新点 2
      预期: 忽略耦合的解在真实模型下可行率下降 (误判可行, 即"坠机")
   6. no_energy     = 无电池约束构造+搜索 (超大电池), 真实电池评估  — 量化电量约束
      预期: 无约束解在真实电池下可行率下降, 体现约束的必要性
✅ 增量评估 vs 全量评估 (创新点 3 的速度对比): 需要重写搜索循环的全量评估版,
   不在 W5 消融矩阵内实现 — 留待 W10+ 论文实验 (当前矩阵 5 行已覆盖
   DEVPLAN W5 要求: 去电量约束 / 固定载重 / 仅 NN)
✅ 指标: 每 variant × 每实例 × 10 runs → 总等效距离 mean±std, 求解时间 mean±std,
   可行率 (feasible runs / total runs)
✅ 实现原则: ablation 通过组合现有纯函数 (construct_nn / local_search_2opt /
   local_search_or_opt / simulate_route_energy) 实现, 不修改 solver 核心路径,
   保持 plan_multistop() 入口签名不变
```
